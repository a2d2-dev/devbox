...
两条保护规则都必须是不受协议端口限制的入站 allow，避免界面端口配置漂移后产生虚假的防锁死保护。任一规则缺失或范围过窄，服务端拒绝生成预览。预览还携带 60 秒定时回滚方案说明。确认端点需要二次确认，但当前构建固定返回 `dryRun: true` 和 `not-applied`，不会执行 `nft`、`iptables` 或 SSH 配置命令。
