...
import SupervisorFace from '../pages/Supervisor'
import { DEVICE, APP_BY_ID, DEFECT_LOGS, QPS_SERIES, LAT_P50, LAT_P95, LAT_P99, PORTS } from '../data/mock'
...
// ═══════════════════════════════════════════════════════════════
function AppShell({ appId, authed, onRequireAuth, onOpenManagement }) {
  ...
  if (appId === 'terminal')  return <TerminalFace/>;
  ...
  if (appId === 'supervisor') return <SupervisorFace/>;
  if (appId === 'supervisor-dashboard') return <IframeFace src={`http://${window.location.hostname}:3802`} title="Supervisor Dashboard"/>;
  return null;
}
