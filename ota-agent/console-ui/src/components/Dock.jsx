...
export function Dock({ apps, onShowDesktop, onFocusApp, onCloseApp, anyVisible,
                authed, authBadge, onToggleAuth, alertBadge }) {
  ...
  return (
    <div style={{
      ...
      <div className="edge-glass"
        ...
        {/* Divider between running apps and trailing utilities */}
        <div style={{ width: 1, height: 28, background: 'rgba(15,23,42,0.08)', margin: '0 4px' }}/>

        {/* Auth utility — fixed, never appears as "running" */}
        <div className="edge-dock-item" onClick={onToggleAuth}
             onMouseEnter={() => setHoverId('__auth')}
             onMouseLeave={() => setHoverId(null)}
             title="身份验证"
             style={{
               position: 'relative',
               width: authed ? 'auto' : 44, height: 44,
               padding: authed ? '0 12px 0 10px' : 0,
               borderRadius: 12,
               background: authed ? '#ecfdf5' : 'rgba(15,23,42,0.04)',
               border: authed ? '1px solid #a7f3d0' : '1px solid transparent',
               color: authed ? '#047857' : T.ink2,
               display: 'flex', alignItems: 'center', justifyContent: 'center',
               cursor: 'pointer', gap: 6,
             }}>
          <Icon name="shield" size={20} stroke={1.7}/>
          {authed && (
            <span className="mono tnum" style={{ fontSize: 11.5, fontWeight: 600 }}>{authBadge}</span>
          )}
          {hoverId === '__auth' && <DockTooltip label={authed ? '已验证身份' : '身份验证'}/>}
        </div>
      </div>
    </div>
  );
