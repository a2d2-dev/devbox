...
import { APP_BY_ID, DEFECT_LOGS, QPS_SERIES, LAT_P50, LAT_P95, LAT_P99 } from '../data/mock'
    ...
    <div style={{ padding: 24, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <Card title="实时性能" action={<span style={{ fontSize: 11.5, color: T.ink3 }}>最近 5 分钟</span>}>
          ...
          <Sparkline data={QPS_SERIES} color={T.blue} width={460} height={70} showAxis max={140}/>
        </Card>
  ...
  return (
    <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 12 }}>
      <Card title="QPS · 最近 30 分钟" action={<span style={{ fontSize: 11.5, color: T.ink3 }}>平均 120 · 峰值 138</span>}>
        <Sparkline data={QPS_SERIES} color={T.blue} width={920} height={140} showAxis max={140}/>
      </Card>

      <Card title="推理延迟 · P50 / P95 / P99">
        <div style={{ position: 'relative', height: 180 }}>
          <Sparkline data={LAT_P99} color={T.red}    width={920} height={180} fill={false} showAxis max={70} style={{ position: 'absolute', top: 0, left: 0 }}/>
          <div style={{ position: 'absolute', top: 0, left: 0 }}>
            <Sparkline data={LAT_P95} color={T.amber} width={920} height={180} fill={false} max={70}/>
          </div>
          <div style={{ position: 'absolute', top: 0, left: 0 }}>
            <Sparkline data={LAT_P50} color={T.green} width={920} height={180} fill={false} max={70}/>
          </div>
        </div>
...
function DefectLogs() {
  ...
  const [lines, setLines] = useState(DEFECT_LOGS);
...
export default function AppDetail({ appId, authed, onRequireAuth }) {
  const app = APP_BY_ID[appId] || APP_BY_ID['defect'];
