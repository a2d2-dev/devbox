...
// Host used to build jump-to-service links — same host the console is served from.
...
function ServiceCard({ proc, onViewLog, onControl }) {
  ...
  const url = proc.port ? `http://${HOST}:${proc.port}` : null;
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 11, color: T.ink3, marginTop: 2, display: 'flex', alignItems: 'center', gap: 5, flexWrap: 'wrap' }}>
            ...
            canJump ? (
              <a href={url} target="_blank" rel="noreferrer"
                ...
                style={{
                  ...
                  color: T.blueDeep, fontWeight: 600, textDecoration: 'none',
                }}>
                :{proc.port}
                <Icon name="external" size={10} stroke={2}/>
              </a>
            ) : (
              <span>:{proc.port}</span>
            )
        ...
        {proc.directory && (
          <div title={proc.directory} style={{
            fontSize: 10.5, color: T.ink3, marginTop: 2,
            display: 'flex', alignItems: 'center', gap: 4,
            whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
          }}>
            <Icon name="folder" size={11} stroke={1.6} style={{ flexShrink: 0, opacity: 0.7 }}/>
            <span style={{ overflow: 'hidden', textOverflow: 'ellipsis' }}>{proc.directory}</span>
          </div>
        )}
      </div>
