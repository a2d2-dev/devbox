...
import { normalizeCategory, deriveAppCenterStatus, matchesAppCenterFilters, sortNewest, sourceTrust } from './appcenter.js';
...
test('应用中心分类归一化覆盖平台与 1Panel 常见标签', () => {
  ...
  expect(normalizeCategory('mail-server')).toBe('mail-server');
});
...
test('应用状态互斥且安装任务优先于静态状态', () => {
  ...
  expect(deriveAppCenterStatus({ installed: true, taskStatus: 'running', taskType: 'remove' })).toBe('installed');
});
...
test('来源信任明确区分官方与社区', () => {
  ...
  expect(sourceTrust({ origin: 'manual' }).trustLabel).toBe('本机自管');
});

test('视图、分类、来源与搜索按交集筛选且搜索不区分大小写', () => {
  const app = { origin: 'catalog', sourceId: 'onepanel', cat: '开发工具', installed: true, name: 'Code Server', desc: 'Browser IDE' };
  expect(matchesAppCenterFilters(app, { view: 'installed', source: 'onepanel', category: '开发工具', query: 'browser ide' })).toBe(true);
  expect(matchesAppCenterFilters(app, { view: 'installed', source: 'platform', category: '开发工具', query: 'code' })).toBe(false);
  expect(matchesAppCenterFilters(app, { view: 'installed', source: 'onepanel', category: '数据存储', query: 'code' })).toBe(false);
});

test('手动安装应用只进入已安装视图', () => {
  const app = { origin: 'manual', installed: true, name: 'nginx hello' };
  expect(matchesAppCenterFilters(app, { view: 'all' })).toBe(false);
  expect(matchesAppCenterFilters(app, { view: 'latest' })).toBe(false);
  expect(matchesAppCenterFilters(app, { view: 'installed', source: 'manual' })).toBe(true);
});
