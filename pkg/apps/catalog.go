...
func findEntryInManifest(m *catalogManifest, appID, version string) (catalogEntry, bool) {
	...
	for _, e := range m.Apps {
		...
		// 版本不匹配但无多版本历史：返回主 entry（版本以 manifest 为准）。
		if len(e.Versions) == 0 {
			return e, true
		}
	}
...
func NewHTTPCatalog(src CatalogSource) (Catalog, error) {
	...
	bc := &baseCatalog{
		...
		resolveFile: func(rel string) (string, error) {
			...
			u := base + "/" + rel
			if err := validateSourceURL(u, src.Insecure); err != nil {
				return "", err
			}
			return u, nil
		},
...
func (c *gitCatalog) refresh(ctx context.Context) error {
	...
	sub := strings.Trim(strings.TrimSpace(c.source.Path), "/")
	newRoot := filepath.Join(tmp, sub)
...
// 再脱敏 token，返回供错误展示。
func gitClone(ctx context.Context, gitBin, url, ref, destDir, token string) (string, error) {
	if gitBin == "" {
		gitBin = "git"
	}
	args := []string{"-c", "credential.helper="} // 禁用凭证助手（不读宿主 keyring）
	if token != "" {
		// Bearer 注入兼容多数 GitLab / GitHub fine-grained PAT；不写 URL 避免 git 回显。
...
// swapDir 原子替换：rm old → rename(new, old)。new 须与 old 在同一文件系统（兄弟目录）。
...
func NewCatalogSet(sources []Catalog, logger *zap.Logger) *CatalogSet {
	...
	// 初始化各 source 快照为 unconfigured/syncing。
	for _, c := range sources {
		cs.snapshots[c.ID()] = CatalogSnapshot{
			SourceID: c.ID(), SourceName: c.ID(), Kind: c.Kind(),
			Status: CatalogStatus{State: CatalogStateSyncing, Message: "首次同步中"},
		}
	}
	return cs
}
...
// NewCatalog 按 CatalogSource.Kind 构造对应 Catalog。cacheRoot 供 git clone 缓存
// （http source 不使用，可空）。
...
func NewCatalogSetFromConfigs(cfgs []CatalogSource, cacheRoot string, logger *zap.Logger) *CatalogSet {
	...
	for _, c := range cfgs {
		...
		if err != nil {
			if logger != nil {
				logger.Warn("catalog source 构造失败，已跳过", zap.String("id", c.ID), zap.String("url", c.URL), zap.Error(err))
			}
			continue
		}
